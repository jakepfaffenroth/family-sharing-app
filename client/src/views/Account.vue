<template>
  ...
  <!-- <transition appear name="slide" mode="out-in">
    <router-view
      ...
      @close-plan-change-modal="closePlanChangeModal"
    >
      <component :is="Component" />
    </router-view>
  </transition> -->
  <div class="flex flex-col w-full px-2 pt-2 sm:px-6 sm:pt-4 xl:px-12 xl:pt-6">
    <div class="flex flex-col sm:flex-row sm:justify-between">
      <a href="/">
        <colorful-logo
          class="self-center sm:self-start"
...
export default {
  name: 'Account',
  components: { ColorfulLogo, AccountSummary, ChangePlan },
  props: {
    owner: {
      type: Object,
      default: () => {
  ...
  setup(props) {
    ...
    // onBeforeMount(async () => {
    //   // planDetails.value = await getCurrentPlan(route.params.ownerId);
    //   getCurrentPlan(route.params.ownerId);
    //   // changeModalBtns(planDetails.value.plan);
    // });
